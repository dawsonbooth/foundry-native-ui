...
import Layout, { baselineWidth, defaultPixelSize } from '../../constants/Layout'
...
/**
 * Converts an em value to pixels for the device the user is on
 * @param {number} size - The em value to get pixels for
 * @param {number} [factor = 1] - The factor to scale by
 * @returns {number} The scaled px value 
 */
 export const emToPx = (size: number, factor = 1) => {
  if (!size) return 0
  
  return defaultPixelSize * (size + (factor * ((Layout.window.width / baselineWidth * size) - size)))
};
